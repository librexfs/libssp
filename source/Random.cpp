...
unsigned long long GenerateRandomNumber(void)
{
    ...
    n = static_cast< long long >( GetRTSC() );

	rnd ^= n^x;

	ROT(rnd,7);

    ret = GetRTSC() + static_cast< unsigned long long >( rnd );
...
namespace {
    ...
    std::mt19937_64 g_randomGenerator( g_randomDevice() );
}

unsigned long long GetRTSC()
{
    std::uniform_int_distribution< unsigned long long > byteDistr( 0x00, std::numeric_limits< unsigned long long >::max() );

    std::lock_guard< std::mutex > _{ g_rnd };
    return byteDistr( g_randomGenerator );
}
